...
async def auto_delete(context: CallbackContext, chat_id: int, message_id: int, delay: int = 60):
    """Delete message after delay (default 60 seconds)"""
...
async def send_and_auto_delete(context: CallbackContext, chat_id: int, text: str, reply_markup=None, delay: int = 60):
    """Send message and schedule auto-delete"""
    message = await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=reply_markup)
    asyncio.create_task(auto_delete(context, chat_id, message.message_id, delay))
    return message

async def send_photo_and_auto_delete(context: CallbackContext, chat_id: int, photo: str, caption: str, delay: int = 60):
    """Send photo and schedule auto-delete"""
...
def get_emoji(key):
    emojis = {
        'name': '👤', 'father_name': '👨', 'mother_name': '👩', 'husband_name': '👨',
        'wife_name': '👩', 'address': '📍', 'village': '🏘️', 'city': '🏙️',
        'district': '🗺️', 'state': '🗺️', 'pincode': '📮', 'mobile': '📱',
        'phone': '📱', 'email': '📧', 'aadhaar': '🪪', 'ration': '📋',
        'id': '🆔', 'number': '🔢', 'dob': '🎂', 'date_of_birth': '🎂',
        'age': '📅', 'gender': '⚥', 'relation': '👥', 'family_id': '👨‍👩‍👧‍👦',
        'card_type': '💳', 'scheme': '📜', 'member_count': '👥', 'members': '👨‍👩‍👧‍👦'
    }
    key_lower = key.lower()
    for emoji_key, emoji in emojis.items():
        if emoji_key in key_lower or key_lower in emoji_key:
            return emoji
    return '📌'
    ...
    try:
        ...
        for key, value in data.items():
            if key not in unwanted and value and str(value).strip():
                ...
                formatted_key = formatted_key.replace('Ration', 'Ration Card No')
        ...
        organized = []
        organized.append("🪪 FAMILY/AADHAAR DETAILS 🪪")
        organized.append("━━━━━━━━━━━━━━━━━━━━━━")
        organized.append("")
        
        # Priority order
        priority_order = [
            'Name', 'Father Name', 'Mother Name', 'Husband Name', 'Wife Name',
            'Address', 'Village', 'City', 'Town', 'District', 'State', 'Pincode',
            'Mobile Number', 'Phone Number', 'Aadhaar Number', 'Ration Card No',
            'Date of Birth', 'Age', 'Gender', 'Relation', 'Family ID',
            'Card Type', 'Scheme', 'Member Count', 'Members'
        ]
        
        used_keys = set()
        for priority in priority_order:
            for key, value in clean_data.items():
                if priority.lower() in key.lower() or key.lower() in priority.lower():
                    organized.append(f"{get_emoji(key)} {key}: {value}")
                    used_keys.add(key)
                    break
        ...
        organized.append("")
        organized.append("⏳ This message will auto-delete in 60 seconds")
...
async def start(update: Update, context: CallbackContext):
    ...
    await send_and_auto_delete(context, update.message.chat_id, welcome_text, delay=120)
...
async def num_command(update: Update, context: CallbackContext):
    ...
    if not context.args:
        await send_and_auto_delete(context, update.message.chat_id, 
    ...
    if result:
        ...
        asyncio.create_task(auto_delete(context, update.message.chat_id, msg.message_id, 60))
...
async def adhaar_command(update: Update, context: CallbackContext):
    ...
    if not context.args:
        await send_and_auto_delete(context, update.message.chat_id,
            "❌ Usage: /adhaar <aadhaar_number/ration_number>\nExample: /adhaar 123456789012")
        return
...
async def img_command(update: Update, context: CallbackContext):
    ...
    if image_url:
        await msg.delete()
        await send_photo_and_auto_delete(context, update.message.chat_id,
            photo=image_url,
...
async def message_handler(update: Update, context: CallbackContext):
    """Handle regular messages for admin operations"""
...
def main():
    ...
    print(f"⏳ Auto-delete enabled: 60 seconds")
