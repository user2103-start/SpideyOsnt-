...
def health():
    return jsonify({"status": "running", "users": len(user_db)}), 200
...
async def setup_webhook():
    ...
    if webhook_url:
        webhook_url = webhook_url.replace('http://', 'https://') + '/webhook'
...
def main():
    ...
    # Check if running on Render/Railway
    is_cloud = os.environ.get('RENDER') or os.environ.get('RAILWAY')
    
    if is_cloud:
        print("🌐 Running in CLOUD mode (Webhook)")
        # Setup webhook
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        webhook_set = loop.run_until_complete(setup_webhook())
        ...
        if webhook_set:
            ...
            print("⚠️ No external URL, falling back to polling")
