...
# Ye Render ke 'Timed Out' error ko khatam karega
app_flask = Flask(__name__)

@app_flask.route('/')
def home():
    return "SPIDEYOSINT Bot is Running!"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "adm_status":
        ...
        if ok: await query.edit_message_text("✅ Access Granted! Use /start to begin."); 
        else: await query.answer("❌ Abhi bhi join nahi kiya bhai!", show_alert=True)
...
async def run_bot():
    ...
    # Flask ko background thread mein chalana taaki Render timeout na de
